...
class UsersDB:
    ...
    def check_remember_me(self, username):
        """
        Check if the user is saved as 'remembere me' and return the result.
        Parameters:
    ...
    def check_mac_address(self, mac_address):
        """
        Checks if the given MAC address exists in the database.
        Parameters:
...
class ScoresDB:
    ...
    def create_table(self):
        '''
        Creates a table named 'scores' in the database if it doesn't already exist. 
        The table has columns for username, lastScore, and mean. The username column is a foreign key referencing the username column in the 'users' table.
        '''
        conn = self.connect_to_db()
        cursor = conn.cursor()
        cursor.execute('''
    ...
    def insert_score(self, username, score, mean):
        '''
        Inserts the given score and updates the mean for a given username into the scores table.
        '''
        self.create_table()
        conn = self.connect_to_db()
        cursor = conn.cursor()
    ...
    def check_user_exists(self, username):
        '''
        Checks if a user with the given username exists in the scores table of the database.
    ...
    def get_mean(self, username):
        '''
        Returns the mean score of the given username.
        '''
        self.create_table()
        conn = self.connect_to_db()
        cursor = conn.cursor()
        ...
        cursor.close()
        conn.close()
        if mean: # if the mean is not None
            return mean
        return 0 # if the mean is None (the is no score for this user)

    def get_last_score(self, username):
        '''
        Returns the last score of the given username.
        '''
        self.create_table()
        conn = self.connect_to_db()
        cursor = conn.cursor()
...
class Message:
    ...
    def decode_json(self, data):
        '''
        The function gets data of bytes type
        The function returns the data as a the list type
        '''
        try:
    ...
    def encode_json(self, data):
        '''
        The function gets data of list type.
        The function returns the data as a bytes type.
        '''
        try:
...
class Sorting_Numbers:
    def __init__(self):
        '''
        Initializes the Sorting_Numbers class.
        '''
        self.numbers_to_sort = [] # saves the numbers to sort
    
    def generate_numbers(self):
        '''
        A function to generate a list of 5 unique random numbers between 1 and 10, shuffle them, and return the list.
        The function returns the list of 5 random numbers.
        The list is saved in the numbers_to_sort variable.
        '''
    ...
    def check_sorted_numbers(self, numbers):
        '''
        Checks if the given numbers are sorted in ascending order.
        '''
...
class Server_Encryption:
    def __init__(self):
        '''
        Initializes the Server_Encryption class.
        '''
        self.key = RSA.generate(1024) # generate a public and private key
        self.public_key = self.key.publickey() # get the public key
        self.private_key = self.key # get the private key
        self.encryption_keys = {} # dictionary to store the encryption keys of the clients
    ...
    def decrypt_data(self, ciphertext, symmetric_key):
        '''
        Decrypts the given ciphertext using the provided symmetric key.
        '''
        cipher = Fernet(symmetric_key)
